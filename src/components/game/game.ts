...
export class Game {
  ...
  private layoutTimer() {
    const timerHTML = createAndAppendHtmlElement(
      this.game,
      "div",
      "game__timer"
    );
    timerHTML.classList.add("timer");
    this.timer_wrapper = createAndAppendHtmlElement(
      timerHTML,
      "div",
      "timer__wrapper"
    );
  ...
  private layoutCards() {
    ...
    this.game__wrapper = createAndAppendHtmlElement(
      this.game,
      "div",
      "game__wrapper"
    );
  ...
  private drawCards(parent: HTMLElement): void {
    ...
    for (let i = 0; i < this._arr_cards.length; i++) {
      ...
      if (
        (i + 1) % this._width_game === 0 &&
        i + 1 !== this._arr_cards.length
      ) {
  ...
  private runTimer(timerHTML: HTMLElement, time_start: Date): void {
    ...
    setInterval(() => {
      if (!this._stop) {
        ...
        const def_time = Math.floor(
          (now_time.getTime() - copy_time_start.getTime()) / 1000
        );
        ...
        const str_time_minute =
          String(this._time.minute).length === 2
            ? String(this._time.minute)
            : String("0" + this._time.minute);
        const str_time_seconds =
          String(this._time.seconds).length === 2
            ? String(this._time.seconds)
            : String("0" + this._time.seconds);
  ...
  private onClickCard(card: Card): void {
    if (
      !card.card.classList.contains("correctly") &&
  ...
  private drawWindowWinner(): void {
    ...
    const window_winner__window = createAndAppendHtmlElement(
      window_winner,
      "div",
      "window_winner__window"
    );
    const text = `Congratulations! You successfully found all matches on ${
      this._time.minute
    }.${
      String(this._time.seconds).length === 2
        ? String(this._time.seconds)
    ...
    const window_winner__btn = createAndAppendHtmlElement(
      window_winner__window,
      "div",
      "window_winner__btn",
      "OK"
  ...
  public get score(): number {
    ...
    if (
      (this._moves - this._wrong_moves) * 100 -
        (this._time.minute * 60 + this._time.seconds) * 10 <
      0
    )
      return 0;
    return (
